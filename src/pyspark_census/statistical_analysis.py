...
class ExploratoryAnalyzer(object):
    ...
    def feature_record(self, iterator):
        records = []
    
        for keys, features in iterator:
            # Handle string keys or tuple keys
            ...
            try:
                records.append({
                    ...
                    "ds": f"{year}-01-01",
                    "year": year,
                    "renter_share": features.get("renter_share"),
                    "education_index": features.get("education_index"),
                    "percent_movers": features.get("percent_movers"),
                    "transit_share": features.get("transit_share"),
                    "y": features.get("raw_index_score")
                })
            except Exception as e:
                print(f"[ERROR] Failed to process {unique_id}, {year}: {e}")
    
        return sorted(records, key=lambda d: int(d['ds']) if d['ds'].isdigit() else -1)
